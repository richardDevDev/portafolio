         			...
         			for(var i=0;i<data.length;i++){
         				switch(data[i].id_sync){
         					...
         					case "6"://configuracion
         						send(formData("syncCodigosLocaltoCloud",data[i].id_sync));
         					break;
...
async function verifyInternet() {
    if (REMOTE_URL === null || REMOTE_URL.trim() === '') {
        hasInternet();
        return;
    }
    try {
        let xhr = new XMLHttpRequest();
        xhr.open("HEAD", REMOTE_URL+"/favicon.ico" , true);
...
function hasInternet(has = null) {
    if (has === null) {
        $('#pinpad-method').hide();
        $('#pinpad-method-disabled').show();
        addLog('Error', 'No se puede comprobar por que la direccion remota no esta definida');
    } else if (has === true) {
        $('#pinpad-method').show();
        $('#pinpad-method-disabled').hide();
        addLog('warn', 'Tienes internet');
    } else {
        $('#pinpad-method').hide();
        $('#pinpad-method-disabled').show();
        addLog('error', 'No tienes internet');
    }
}

$(() => {
    let global_file_inputs = document.getElementsByClassName('custom-file-input');
    if (global_file_inputs.length > 0) {
        for (let i = 0; i < global_file_inputs.length; i++) {
            global_file_inputs[i].addEventListener('change', (e) => {
                let fileName = 'Selecciona un archivo';
                let this_input = e.target;
                let output = null;
                if (this_input.files[0] !== undefined) {
                    if (this_input.files.length > 1) {
                        fileName = 'Se seleccionaron ' + this_input.files.length + ' archivos';
                        output = document.getElementById('img-' + this_input.getAttribute('data-id'));
                        if (output !== null) {
                            output.removeAttribute('src');
                        }
                    } else {
                        fileName = this_input.files[0].name;
                        if (this_input.files[0].name.endsWith('.webp') ||
                            this_input.files[0].name.endsWith('.png') ||
                            this_input.files[0].name.endsWith('.jpg') ||
                            this_input.files[0].name.endsWith('.jpeg') ||
                            this_input.files[0].name.endsWith('.jfif') ||
                            this_input.files[0].name.endsWith('.gif')
                        ) {
                            loadFile(e, e.target);
                            let b_str = document.getElementById('#bx-str-file');
                            if (b_str !== null) {
                                b_str.innerText = '';
                            }
                        } else {
                            output = document.getElementById('img-' + this_input.getAttribute('data-id'));
                            if (output !== null) {
                                if (this_input.files[0].name.endsWith('.pdf')) {
                                    output.attr('src', BASE_URL + '/app/includes/images/defaults/svg/pdf.svg')
                                } else {
                                    output.attr('src', BASE_URL + '/app/includes/images/defaults/svg/nd.min.svg')
                                }

                                let b_str = document.getElementById('#bx-str-file');
                                if (b_str !== null) {
                                    b_str.innerText = fileName;
                                }
                            }
                        }
                    }
                } else {
                    output = document.getElementById('img-' + this_input.getAttribute('id'));
                    if (output !== null) {
                        output.setAttribute('src', BASE_URL + '/app/includes/images/defaults/svg/nd.min.svg');
                        let b_str = document.getElementById('#bx-str-file');
                        if (b_str !== null) {
                            b_str.innerText = '';
                        }
                ...
                let txt_info = this_input.parentNode;
                let txt_info_span = txt_info.querySelector('.custom-file-label');
                if (txt_info_span !== null) {
                    txt_info_span.innerText = fileName;
                }
            });
        }
    }
    let loadFile = function (event, y) {
        let output = document.getElementById('img-' + y.getAttribute('id'));
        output.src = URL.createObjectURL(y.files[0]);
        output.onload = function () {
            URL.revokeObjectURL(output.src) // liberar memoria
        }
    };

    let global_images_change_alone = document.getElementsByClassName('img-change-alone');
    if (global_images_change_alone.length > 0) {
        for (let i = 0; i < global_images_change_alone.length; i++) {
            global_images_change_alone[i].addEventListener('change', (e) => {
                let this_input = e.target;
                let uuid_img = this_input.getAttribute('data-uuid');
                let id = this_input.getAttribute('data-id');

                if (this_input.files.length < 1) {
                    return;
                } else if (this_input.files.length > 1) {
                    Swal.fire('Advertencia', 'No se pueden actualizar más de una imagen a la vez por este metodo', 'warning');
                    return;
                } else if (uuid_img === null || id === null) {
                    Swal.fire('Error', 'No fue posible actualizar debido a que no se tienen los parametros requeridos', 'error');
                    return;
                }

                let data = new FormData();
                let name = this_input.getAttribute('name');
                if (name === null) {
                    name = 'link';
                }
                data.append("file", this_input.files[0]);
                data.append("uuid", uuid_img);
                data.append("id", id);
                data.append('handler', 'upload-file-app');
                send(data);
            })
        }
    }
