...
if(typeof __type_project__ === "undefined"){
 ...
 */
function owlExecuteTestEnginesMatrix(motors=null) {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            owlEnginesMatrix(engine1, engine2, "01");
            $("#testingDiv").text("Motor:"+owl_aux_iteration);
        }, 300);
        if (owl_aux_iteration < 90) {
...
function owlReturnChange() {
    ...
    console.warn("se entrego:",change_returned_verified);
    //console.log(owl_change,(totalMonedasTubos+totalBilletesRecycler));
    if(owl_change > (totalMonedasTubos+totalBilletesRecycler)){	
        change_returned_verified = (totalMonedasTubos)+totalBilletesRecycler;
    	addLog("warn","El cambio es mas de lo que se puede entregar",1);
        extOwlRetireBill(true);
	}else if(notifyChangeNotDispensed){
        addLog("warn","El cambio es mas de lo que se puede entregar, mostrando alerta en pantalla",2);
...
function owlReturnChangeComprobator(change) {
    ...
    change_returned_verified = owl_change;
    if(owl_change > (totalMonedasTubos)+totalBilletesRecycler){ 
        addLog("warn","El cambio es mas de lo que se puede entregar",3);
        return false;
...
function owlHasToReturnChange(price_to_eval) {
    ...
    if((price_to_eval > (totalMonedasTubos)+totalBilletesRecycler) || (notifyChangeNotDispensed)){	
    	addLog("warn","El cambio es mas de lo que se puede entregar",4);
        return false;
	}
    return true;
}
